...
// OutstandingSDEAM resolves the amount of outstanding sDEAM tokens
// minted for this account.
func (del Delegation) OutstandingSDEAM() (hexutil.Big, error) {
	val, err := repository.R().DelegationOutstandingSDEAM(&del.Address, del.Delegation.ToStakerId)
	if err != nil {
		return hexutil.Big{}, err
	}
	return *val, nil
