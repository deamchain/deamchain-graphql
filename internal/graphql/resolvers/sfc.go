...
// CurrentEpoch resolves the id of the current epoch of the DEAM blockchain.
func (rs *rootResolver) CurrentEpoch() (hexutil.Uint64, error) {
	return repository.R().CurrentEpoch()
}

// LastStakerId resolves the last staker id in DEAM blockchain.
...
// StakersNum resolves the number of stakers in DEAM blockchain.
