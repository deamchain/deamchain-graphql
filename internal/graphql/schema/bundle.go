...
type DefiToken {
    ...
    # canWrapDEAM signals if the token can be used
    # to wrap native DEAM tokens for DeFi trading.
    canWrapDEAM: Boolean!
...
type Delegation {
    ...
    # outstandingSDEAM represents the amount of sDEAM tokens representing
    # the tokenized stake minted and un-repaid on this delegation.
    outstandingSDEAM: BigInt!

    # tokenizerAllowedToWithdraw indicates if the stake tokenizer allows the stake
    # to be withdrawn. That means all the sDEAM tokens have been repaid and the sDEAM
...
type EstimatedRewards {
    # Amount of DEAM tokens expected to be staked for the calculation.
    staked: Long!

    # dailyReward represents amount of DEAM tokens estimated
    # to be rewarded for staked amount in average per day.
    dailyReward: BigInt!

    # weeklyReward represents amount of DEAM tokens estimated
    # to be rewarded for staked amount in average per week.
    weeklyReward: BigInt!

    # monthlyReward represents amount of DEAM tokens estimated
    # to be rewarded for staked amount in average per month.
    monthlyReward: BigInt!

    # yearlyReward represents amount of DEAM tokens estimated
    ...
    # Total amount of staked DEAM tokens used for the calculation in WEI units.
    ...
    # Information about the last sealed epoch of the DEAM blockchain.
    # The epoch provides useful information about total DEAM supply,
...
type Query {
    ...
    # Total number of accounts active on the DEAM blockchain.
    accountsActive:Long!
    ...
    # Get the id of the current epoch of the DEAM blockchain.
    currentEpoch:Long!
    ...
    # The last staker id in DEAM blockchain.
    lastStakerId: Long!

    # The number of stakers in DEAM blockchain.
    stakersNum: Long!
    ...
    # Get price details of the DEAM blockchain token for the given target symbols.
    price(to:String!):Price!

    # Get calculated staking rewards for an account or given
    # staking amount in DEAM tokens.
    ...
    # by the Uniswap Core contract on DEAM blockchain.
