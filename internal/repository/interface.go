...
type Repository interface {
	// Account returns account at DEAM blockchain for an address, nil if not found.
	Account(*common.Address) (*types.Account, error)

	// AccountBalance returns the current balance of an account at DEAM blockchain.
	AccountBalance(*common.Address) (*hexutil.Big, error)

	// AccountNonce returns the current number of sent transactions of an account at DEAM blockchain.
	AccountNonce(*common.Address) (*hexutil.Uint64, error)

	// AccountTransactions returns list of transaction hashes for account at DEAM blockchain.
	//
	...
	// BlockHeight returns the current height of the DEAM blockchain in blocks.
	...
	// BlockByNumber returns a block at DEAM blockchain represented by a number.
	...
	// BlockByHash returns a block at DEAM blockchain represented by a hash.
	...
	// Contracts returns list of smart contracts at DEAM blockchain.
	...
	// LoadTransaction returns a transaction at DEAM blockchain
	// by a hash loaded directly from the node.
	LoadTransaction(hash *common.Hash) (*types.Transaction, error)

	// Transaction returns a transaction at DEAM blockchain by a hash, nil if not found.
	Transaction(*common.Hash) (*types.Transaction, error)

	// Transactions returns list of transaction hashes at DEAM blockchain.
	...
	// LastValidatorId returns the last validator id in DEAM blockchain.
	LastValidatorId() (uint64, error)

	// ValidatorsCount returns the number of stakers in DEAM blockchain.
	...
	DelegationOutstandingSDEAM(*common.Address, *hexutil.Big) (*hexutil.Big, error)
