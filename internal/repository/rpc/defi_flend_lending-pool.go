...
func (deam *DeamBridge) FLendGetLendingPool() (*contracts.ILendingPool, error) {
	// get the lending pool contract
	lp, err := contracts.NewILendingPool(deam.fLendCfg.lendigPoolAddress, deam.eth)
	if err != nil {
		deam.log.Errorf("Can not get lending pool contract on address %s; %s", deam.fLendCfg.lendigPoolAddress.String(), err.Error())
		return nil, err
	}
	return lp, nil
}
...
func (deam *DeamBridge) FLendGetLendingPoolReserveData(assetAddress *common.Address) (*types.ReserveData, error) {

	// get the lending pool contract
	lp, err := deam.FLendGetLendingPool()
	if err != nil {
		deam.log.Errorf("Can not access lending pool %s", err.Error())
		return nil, err
	}

	rd, err := lp.GetReserveData(&bind.CallOpts{}, *assetAddress)
	if err != nil {
		deam.log.Errorf("Cannot get reserve data for asset %s: %s", assetAddress.String(), err.Error())
		return nil, err
	}
...
func (deam *DeamBridge) FLendGetReserveList() ([]common.Address, error) {
	...
	if err != nil {
		deam.log.Errorf("Cannot get reserves list: %s", err.Error())
		return nil, err
	}
	return rl, nil
}
...
func (deam *DeamBridge) FLendGetUserAccountData(userAddress *common.Address) (*types.FLendUserAccountData, error) {
	...
	if err != nil {
		deam.log.Errorf("Cannot get user account data for address %s: %s", userAddress.String(), err.Error())
		return nil, err
	}

	uc, err := lp.GetUserConfiguration(&bind.CallOpts{}, *userAddress)
	if err != nil {
		deam.log.Errorf("Cannot get user account configuration data for address %s: %s", userAddress.String(), err.Error())
		return nil, err
	}
...
func (deam *DeamBridge) FLendGetUserDepositHistory(userAddress *common.Address, assetAddress *common.Address) ([]*types.FLendDeposit, error) {
	// create user filter
	...
	if err != nil {
		deam.log.Errorf("can not filter lending pool deposit logs: %s", err.Error())
		return nil, err
	}
	...
	for fdi.Next() {
		...
		blk, err := deam.BlockByHash(&blkHash)
		if err != nil {
			deam.log.Errorf("fLend block with hash %s was not found: %s", blkHash, err.Error())
			continue
		}
