...
func (deam *DeamBridge) FMintAccount(owner *common.Address) (*types.FMintAccount, error) {
	// make the container
	var err error
	da := types.FMintAccount{Address: *owner}

	// load list of collateral tokens
	da.CollateralList, err = deam.DefiTokenList()
	if err != nil {
		deam.log.Errorf("collateral tokens list loader failed; %s", err.Error())
		return nil, err
	}
	...
	da.CollateralValue, da.DebtValue, err = deam.fMintAccountValue(*owner)
	if err != nil {
		deam.log.Errorf("can not pull account tokens value; %s", err.Error())
		return nil, err
	}
...
func (deam *DeamBridge) FMintPoolBalance(pool *contracts.DeFiTokenStorage, owner *common.Address, token *common.Address) (hexutil.Big, error) {
	// get the collateral token balance
	val, err := pool.BalanceOf(nil, *owner, *token)
	if err != nil {
		deam.log.Debugf("pool balance failed on token %s, account %s; %s", token.String(), owner.String(), err.Error())
		return hexutil.Big{}, err
	}

	// do we have a value?
	if val == nil {
		deam.log.Debugf("token %s balance not available for owner %s", token.String(), owner.String())
...
func (deam *DeamBridge) FMintTokenBalance(owner *common.Address, token *common.Address, tp types.DefiTokenType) (hexutil.Big, error) {
	var err error
	...
	case types.DefiTokenTypeCollateral:
		pool, err = deam.fMintCfg.fMintCollateralPool()
	case types.DefiTokenTypeDebt:
		pool, err = deam.fMintCfg.fMintDebtPool()
	}

	// error in pool loading?
	if err != nil {
		deam.log.Debugf("token storage pool failed to load; %s", err.Error())
		return hexutil.Big{}, err
	}

	// do we have the pool?
	if pool == nil {
		deam.log.Debugf("token storage pool not available")
		return hexutil.Big{}, nil
	}

	return deam.FMintPoolBalance(pool, owner, token)
}

// FMintTokenTotalBalance loads total balance of a single DeFi token by it's address.
func (deam *DeamBridge) FMintTokenTotalBalance(token *common.Address, tp types.DefiTokenType) (hexutil.Big, error) {
	var err error
	var pool *contracts.DeFiTokenStorage
	...
	if err != nil {
		deam.log.Debugf("pool total balance failed on token %s", token.String(), err.Error())
		return hexutil.Big{}, err
	}

	// do we have a value?
	if val == nil {
		deam.log.Debugf("token %s total balance not available", token.String())
		return hexutil.Big{}, nil
	}
...
func (deam *DeamBridge) FMintTokenValue(owner *common.Address, token *common.Address, tp types.DefiTokenType) (hexutil.Big, error) {
	// get the balance
	balance, err := deam.FMintTokenBalance(owner, token, tp)
	if err != nil {
		deam.log.Errorf("token %s balance unknown; %s", token.String(), err.Error())
		return hexutil.Big{}, err
	}

	// get the price for the given token from oracle
	val, err := deam.FMintTokenPrice(token)
	if err != nil {
		deam.log.Errorf("token %s price not available; %s", token.String(), err.Error())
		return hexutil.Big{}, err
	}
...
func (deam *DeamBridge) FMintTokenPrice(token *common.Address) (hexutil.Big, error) {
	// get the price oracle address
	oracle, err := deam.fMintCfg.priceOracleProxyContract()
	if err != nil {
		return hexutil.Big{}, err
	}
	...
	if err != nil {
		deam.log.Errorf("price not available for token %s; %s", token.String(), err.Error())
		return hexutil.Big{}, nil
	}

	// do we have the value?
	if val == nil {
		deam.log.Debugf("token %s has no value", token.String())
		return hexutil.Big{}, nil
	}
...
func (deam *DeamBridge) fMintAccountValue(owner common.Address) (hexutil.Big, hexutil.Big, error) {
	...
	if err != nil {
		deam.log.Errorf("joined collateral value loader failed")
		return hexutil.Big{}, hexutil.Big{}, err
	}
	...
	if err != nil {
		deam.log.Errorf("joined debt value loader failed")
		return hexutil.Big{}, hexutil.Big{}, err
	}
...
func (deam *DeamBridge) FMintRewardsEarned(addr *common.Address) (hexutil.Big, error) {
	// connect the contract
	contract, err := deam.fMintCfg.fMintRewardsDistribution()
	if err != nil {
		return hexutil.Big{}, err
	}

	// get the block height
	bl, err := deam.BlockHeight()
	if err != nil {
		return hexutil.Big{}, err
	}
	...
	if err != nil {
		deam.log.Errorf("can not calculate earned rewards; %s", err.Error())
		return hexutil.Big{}, err
	}
...
// accumulated on the account for the excessive collateral deposits.
func (deam *DeamBridge) FMintRewardsStashed(addr *common.Address) (hexutil.Big, error) {
	// connect the contract
	contract, err := deam.fMintCfg.fMintRewardsDistribution()
	if err != nil {
		return hexutil.Big{}, err
	...
	if err != nil {
		deam.log.Errorf("can not calculate stashed rewards; %s", err.Error())
		return hexutil.Big{}, err
	}
...
func (deam *DeamBridge) FMintCanClaimRewards(addr *common.Address) (bool, error) {
	// connect the contract
	contract, err := deam.fMintCfg.fMintMinterContract()
	if err != nil {
		deam.log.Errorf("rewards claim check failed; %s", err.Error())
		return false, err
	}
	...
	if err != nil {
		deam.log.Errorf("can not check rewards claim flag; %s", err.Error())
		return false, err
	}
...
func (deam *DeamBridge) FMintCanReceiveRewards(addr *common.Address) (bool, error) {
	// connect the contract
	contract, err := deam.fMintCfg.fMintMinterContract()
	if err != nil {
		deam.log.Errorf("rewards eligibility check failed; %s", err.Error())
		return false, err
	}
	...
	if err != nil {
		deam.log.Errorf("can not check rewards eligibility flag; %s", err.Error())
		return false, err
	}
...
func (deam *DeamBridge) FMintCanPushRewards() (bool, error) {
	// connect the contract
	contract, err := deam.fMintCfg.fMintRewardsDistribution()
	if err != nil {
		return false, err
	...
	if err != nil {
		deam.log.Errorf("can not check rewards last push; %s", err.Error())
		return false, err
	}
