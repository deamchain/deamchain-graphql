...
func (deam *DeamBridge) Erc1155Uri(token *common.Address, tokenId *big.Int) (string, error) {
	// connect the contract
	contract, err := contracts.NewERC1155(*token, deam.eth)
	if err != nil {
		deam.log.Errorf("can not contact ERC1155 contract; %s", err.Error())
		return "", err
	}
	...
	if err != nil {
		deam.log.Errorf("ERC1155 token %s/%s URI not available; %s", token.String(), tokenId.String(), err.Error())
		return "", err
	}
...
func (deam *DeamBridge) Erc1155BalanceOf(token *common.Address, owner *common.Address, tokenId *big.Int) (*big.Int, error) {
	// connect the contract
	contract, err := contracts.NewERC1155(*token, deam.eth)
	if err != nil {
		deam.log.Errorf("can not contact ERC1155 contract; %s", err.Error())
	...
	if err != nil {
		deam.log.Errorf("can not get ERC1155 %s/%s balance for %s; %s", token.String(), tokenId.String(), owner.String(), err.Error())
		return nil, err
	}
...
func (deam *DeamBridge) Erc1155BalanceOfBatch(token *common.Address, owners *[]common.Address, tokenIds []*big.Int) ([]*big.Int, error) {
	// connect the contract
	contract, err := contracts.NewERC1155(*token, deam.eth)
	if err != nil {
		deam.log.Errorf("can not contact ERC1155 contract; %s", err.Error())
	...
	if err != nil {
		deam.log.Errorf("can not get ERC1155 batch balance for %s; %s", token.String(), err.Error())
		return nil, err
	}
...
func (deam *DeamBridge) Erc1155IsApprovedForAll(token *common.Address, owner *common.Address, operator *common.Address) (bool, error) {
	// connect the contract
	contract, err := contracts.NewERC1155(*token, deam.eth)
	if err != nil {
		deam.log.Errorf("can not contact ERC1155 contract; %s", err.Error())
	...
	if err != nil {
		deam.log.Errorf("can not get ERC1155 %s approved-for-all status for owner %s and operator %s; %s", token.String(), owner.String(), operator.String(), err.Error())
		return false, err
	}
