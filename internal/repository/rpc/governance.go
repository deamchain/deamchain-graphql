...
func (deam *DeamBridge) GovernanceProposalsCount(gov *common.Address) (hexutil.Big, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, deam.eth)
	if err != nil {
		deam.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	if err != nil {
		deam.log.Errorf("can not count governance %s proposals; %s", gov.String(), err.Error())
		return hexutil.Big{}, err
	}
...
// specified by its id.
func (deam *DeamBridge) GovernanceProposal(gov *common.Address, id *hexutil.Big) (*types.GovernanceProposal, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, deam.eth)
	if err != nil {
		deam.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	// get the detail
	gp, err := deam.governanceProposalDetail(gc, gov, id.ToInt())
	if err != nil {
		deam.log.Errorf("proposal %d not available in %s; %s", id.ToInt().Uint64(), gov.String(), err.Error())
		return nil, err
	}
...
// GovernanceProposal provides a detail of Proposal of a governance contract
// specified by its id.
func (deam *DeamBridge) governanceProposalDetail(gc *contracts.Governance, govId *common.Address, id *big.Int) (*types.GovernanceProposal, error) {
	...
	ext, err := deam.GovernanceProposalDetails(&data.ProposalContract)
	if err != nil {
		return nil, err
	}
...
// specified by its id.
func (deam *DeamBridge) GovernanceProposalState(gov *common.Address, id *hexutil.Big) (*types.GovernanceProposalState, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, deam.eth)
	if err != nil {
		deam.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	if err != nil {
		deam.log.Errorf("can not access governance %s proposal %d state; %s", gov.String(), id.ToInt().Int64(), err.Error())
		return nil, err
	}
...
// specified by its id.
func (deam *DeamBridge) GovernanceProposalDetails(prop *common.Address) (*govProposalExtended, error) {
	// get the proposal contract
	pp, err := contracts.NewGovernanceProposal(*prop, deam.eth)
	if err != nil {
		deam.log.Errorf("can not access governance proposal %s; %s", prop.String(), err.Error())
		return nil, err
	}
	...
	if err != nil {
		deam.log.Errorf("governance proposal %s name not available; %s", prop.String(), err.Error())
		return nil, err
	}
	...
	if err != nil {
		deam.log.Errorf("governance proposal %s description not available; %s", prop.String(), err.Error())
		return nil, err
	}
...
func (deam *DeamBridge) GovernanceOptionState(gov *common.Address, propId *hexutil.Big, optId *hexutil.Big) (*types.GovernanceOptionState, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, deam.eth)
	if err != nil {
		deam.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	gs, err := deam.GovernanceOptionStateById(gc, propId, optId)
	if err != nil {
		deam.log.Errorf("governance %s proposal #%d state #%d not available; %s",
...
func (deam *DeamBridge) GovernanceOptionStates(gov *common.Address, propId *hexutil.Big, optRange int) ([]*types.GovernanceOptionState, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, deam.eth)
	if err != nil {
		deam.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	for i := 0; i < optRange; i++ {
		...
		res[i], err = deam.GovernanceOptionStateById(gc, propId, (*hexutil.Big)(big.NewInt(int64(i))))
		if err != nil {
			deam.log.Errorf("unknown option #%d on governance %s; %s", i, gov.String(), err.Error())
			break
		}
	}
	return res, nil
...
func (deam *DeamBridge) GovernanceOptionStateById(gc *contracts.Governance, propId *hexutil.Big, optId *hexutil.Big) (*types.GovernanceOptionState, error) {
	// get the state
...
func (deam *DeamBridge) GovernanceVote(
	gov *common.Address,
	propId *hexutil.Big,
	from *common.Address,
	...
	if err != nil {
		deam.log.Errorf("can not access vote of %s on governance %s; %s", from.String(), gov.String(), err.Error())
		return nil, err
	}
...
func (deam *DeamBridge) GovernanceProposalsBy(gov *common.Address) ([]*types.GovernanceProposal, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, deam.eth)
	if err != nil {
		deam.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	// log what we do
	deam.log.Noticef("loading %d proposals of %s", maxProposalId.Uint64(), gov.String())
	...
	for i := maxProposalId.Int64(); i > 0; i-- {
		...
		gp, err := deam.governanceProposalDetail(gc, gov, big.NewInt(i))
		if err != nil {
			deam.log.Errorf("can not access governance %s proposal %d; %s", gov.String(), i, err.Error())
			return nil, err
		}

		// keep the proposal in the list
		deam.log.Noticef("found proposal #%d on %s", gp.Id.ToInt().Uint64(), gov.String())
		result = append(result, gp)
	}

	// log what we do
	deam.log.Debugf("%d proposals of %s loaded", maxProposalId.Uint64(), gov.String())
	return result, nil
}

// GovernanceProposalFee returns the fee payable for a new proposal
// in given Governance contract context.
func (deam *DeamBridge) GovernanceProposalFee(gov *common.Address) (hexutil.Big, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, deam.eth)
	if err != nil {
		deam.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	if err != nil {
		deam.log.Errorf("governance %s fee not available; %s", gov.String(), err.Error())
		return hexutil.Big{}, err
	}
...
func (deam *DeamBridge) GovernanceTotalWeight(ge *common.Address) (*hexutil.Big, error) {
	// get the contract
	goe, err := contracts.NewGovernable(*ge, deam.eth)
	if err != nil {
		deam.log.Errorf("can not access governable adapter %s; %s", ge.String(), err.Error())
		return nil, err
	}

	// get the total available voting weight
	w, err := goe.GetTotalWeight(deam.DefaultCallOpts())
	if err != nil {
		deam.log.Errorf("total weight not available on adapter %s; %s", ge.String(), err.Error())
		return nil, err
	}
