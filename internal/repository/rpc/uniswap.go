...
func (deam *DeamBridge) NativeTokenAddress() (*common.Address, error) {
	// get the router contract if possible
	contract, err := contracts.NewUniswapRouter(deam.uniswapConfig.Router, deam.eth)
	if err != nil {
		deam.log.Errorf("Uniswap router contract not found; %s", err.Error())
		return nil, err
	}
	...
	if err != nil {
		deam.log.Errorf("Native token address not available; %s", err.Error())
		return nil, err
	}
...
func (deam *DeamBridge) UniswapPair(tokenA *common.Address, tokenB *common.Address) (*common.Address, error) {
	// get the router contract if possible
	contract, err := contracts.NewUniswapFactory(deam.uniswapConfig.Core, deam.eth)
	if err != nil {
		deam.log.Errorf("Uniswap factory contract not found; %s", err.Error())
		return nil, err
	}
	...
	if err != nil {
		deam.log.Errorf("Uniswap pair not found for tokens %s and %s; %s", tokenA.String(), tokenB.String(), err.Error())
		return nil, err
	}
...
func (deam *DeamBridge) UniswapPairs(whiteListedOnly bool) ([]common.Address, error) {
	// get the router contract if possible
	contract, err := contracts.NewUniswapFactory(deam.uniswapConfig.Core, deam.eth)
	if err != nil {
		deam.log.Errorf("Uniswap factory contract not found; %s", err.Error())
	...
	if err != nil || length == nil {
		deam.log.Error("Uniswap pairs array length not available")
		return nil, err
	}
	...
	for i := uint64(0); i < length.Uint64(); i++ {
		...
		if err != nil {
			deam.log.Errorf("error loading Uniswap pair; %s", err.Error())
			continue
		}

		// check the pair (is it on the white list?)
		// add the address to the list, if it's ok
		if !whiteListedOnly || deam.isUniswapPairWhitelisted(&adr) {
...
func (deam *DeamBridge) isUniswapPairWhitelisted(pair *common.Address) bool {
	// decode the pair address
	pairAddr := pair.String()

	// check the pair address against all the white listed pairs in config
	for _, addr := range deam.uniswapConfig.PairsWhiteList {
...
func (deam *DeamBridge) UniswapQuoteInput(
	amountA hexutil.Big,
	...
	if err != nil {
		deam.log.Errorf("can not calculate Uniswap quote; %s", err.Error())
		return hexutil.Big{}, err
	}
...
func (deam *DeamBridge) UniswapAmountsOut(amountIn hexutil.Big, tokens []common.Address) ([]hexutil.Big, error) {
	// get the router contract if possible
	contract, err := contracts.NewUniswapRouter(deam.uniswapConfig.Router, deam.eth)
	if err != nil {
		deam.log.Errorf("Uniswap router contract not found; %s", err.Error())
	...
	return deam.uniswapLoadAmounts(contract.GetAmountsOut, (*big.Int)(&amountIn), tokens)
}
...
func (deam *DeamBridge) UniswapAmountsIn(amountOut hexutil.Big, tokens []common.Address) ([]hexutil.Big, error) {
	// get the router contract if possible
	contract, err := contracts.NewUniswapRouter(deam.uniswapConfig.Router, deam.eth)
	if err != nil {
		deam.log.Errorf("Uniswap router contract not found; %s", err.Error())
	...
	return deam.uniswapLoadAmounts(contract.GetAmountsIn, (*big.Int)(&amountOut), tokens)
}

// uniswapLoadAmounts loads a list of swap amounts using the given loader function
// of the Uniswap router contract.
func (deam *DeamBridge) uniswapLoadAmounts(
	...
	if err != nil {
		deam.log.Errorf("can not load swap amounts; %s", err.Error())
		return nil, err
	}
...
func (deam *DeamBridge) UniswapTokens(pair *common.Address) ([]common.Address, error) {
	// get the pair contract if possible
	contract, err := contracts.NewUniswapPair(*pair, deam.eth)
	if err != nil {
		deam.log.Errorf("Uniswap pair %s not found; %s", pair.String(), err.Error())
		return nil, err
	}
	...
	if err != nil {
		deam.log.Errorf("Uniswap pair %s token A not found; %s", pair.String(), err.Error())
		return nil, err
	}
	...
	if err != nil {
		deam.log.Errorf("Uniswap pair %s token B not found; %s", pair.String(), err.Error())
		return nil, err
	}
...
func (deam *DeamBridge) UniswapReserves(pair *common.Address) ([]hexutil.Big, error) {
	// get the reserves record from the contract
	rs, err := deam.uniswapReservesRecord(pair)
	if err != nil {
		deam.log.Errorf("Uniswap pair %s reserves not available; %s", pair.String(), err.Error())
		return nil, err
	}
...
func (deam *DeamBridge) UniswapReservesTimeStamp(pair *common.Address) (hexutil.Uint64, error) {
	// get the reserves record from the contract
	rs, err := deam.uniswapReservesRecord(pair)
	if err != nil {
		deam.log.Errorf("Uniswap pair %s reserves not available; %s", pair.String(), err.Error())
...
func (deam *DeamBridge) UniswapCumulativePrices(pair *common.Address) ([]hexutil.Big, error) {
	// get the pair contract if possible
	contract, err := contracts.NewUniswapPair(*pair, deam.eth)
	if err != nil {
		deam.log.Errorf("Uniswap pair %s not found; %s", pair.String(), err.Error())
	...
	if err != nil {
		deam.log.Errorf("Uniswap pair %s cumulative price A not available; %s", pair.String(), err.Error())
		return nil, err
	}
	...
	if err != nil {
		deam.log.Errorf("Uniswap pair %s cumulative price B not available; %s", pair.String(), err.Error())
		return nil, err
	}
...
func (deam *DeamBridge) uniswapReservesRecord(pair *common.Address) (*struct {
	Reserve0           *big.Int
	Reserve1           *big.Int
...
func (deam *DeamBridge) UniswapLastKValue(pair *common.Address) (hexutil.Big, error) {
	// get the pair contract if possible
	contract, err := contracts.NewUniswapPair(*pair, deam.eth)
	if err != nil {
		deam.log.Errorf("Uniswap pair %s not found; %s", pair.String(), err.Error())
	...
	if err != nil {
		deam.log.Errorf("Uniswap pair %s coefficient K not available; %s", pair.String(), err.Error())
		return hexutil.Big{}, err
	}
...
func (deam *DeamBridge) UniswapPairContract(pairAddres *common.Address) (*contracts.UniswapPair, error) {
	contract, err := contracts.NewUniswapPair(*pairAddres, deam.eth)
	if err != nil {
		deam.log.Errorf("Uniswap pair contract %s not found; %s", pairAddres.String(), err.Error())
		return nil, err
	}
	return contract, nil
}
...
func (deam *DeamBridge) UniswapFactoryContract() (*contracts.UniswapFactory, error) {
	// get the router contract if possible
	contract, err := contracts.NewUniswapFactory(deam.uniswapConfig.Core, deam.eth)
	if err != nil {
		deam.log.Errorf("Uniswap factory contract not found; %s", err.Error())
