...
func (deam *DeamBridge) GasPrice() (hexutil.Big, error) {
	// keep track of the operation
	deam.log.Debugf("checking current gas price")

	// call for data
	var price hexutil.Big
	err := deam.rpc.Call(&price, "deam_gasPrice")
	if err != nil {
		deam.log.Error("current gas price could not be obtained")
		return price, err
	}
...
func (deam *DeamBridge) GasEstimate(trx *struct {
	From  *common.Address
	To    *common.Address
	Value *hexutil.Big
	Data  *string
	...
	// keep track of the operation
	deam.log.Debugf("calling for gas amount estimation")

	var val hexutil.Uint64
	err := deam.rpc.Call(&val, "deam_estimateGas", trx)
	if err != nil {
		// missing required argument? incompatibility between old and new RPC API
		if strings.Contains(err.Error(), "missing value") {
			return deam.GasEstimateWithBlock(trx)
		}

		// return error
		deam.log.Errorf("can not estimate gas; %s", err.Error())
		return nil, err
	}
...
func (deam *DeamBridge) GasEstimateWithBlock(trx *struct {
	From  *common.Address
	To    *common.Address
	Value *hexutil.Big
	Data  *string
	...
	// keep track of the operation
	deam.log.Debugf("calling for gas amount estimation with block details")

	var val hexutil.Uint64
	err := deam.rpc.Call(&val, "deam_estimateGas", trx, BlockTypeLatest)
	if err != nil {
		// return error
		deam.log.Errorf("can not estimate gas; %s", err.Error())
		return nil, err
