...
import (
	"deamchain-graphql/internal/repository/db"
...
func (p *proxy) DelegationOutstandingSDEAM(addr *common.Address, toStaker *hexutil.Big) (*hexutil.Big, error) {
	val, err := p.rpc.DelegationOutstandingSDEAM(addr, toStaker.ToInt())
	if err != nil {
		return nil, err
	}
	return (*hexutil.Big)(val), nil
